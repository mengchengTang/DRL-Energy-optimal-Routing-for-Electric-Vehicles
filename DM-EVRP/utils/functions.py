...
def load_problem(name):
    from problems import EVRP, EVRPTW
    problem = {
        'evrp': EVRP,
        "evrptw":EVRPTW
...
def sample_many(inner_func, get_cost_func, input, batch_rep=1, iter_rep=1):
    ...
    for i in range(iter_rep):
        _log_p, _log_p_veh, pi, veh_list, tour_1, tour_2, tour_3, tour_4, tour_5 = inner_func(input, )
        #print('pi', pi.size())

        cost, mask = get_cost_func(input, pi, veh_list, tour_1, tour_2, tour_3, tour_4, tour_5)
...
def parse_file(lines):
    coord_section_line = lines.index("NODE_COORD_SECTION\n") + 1
    demand_section_line = lines.index("DEMAND_SECTION\n") + 1
    depot_section_line = lines.index("DEPOT_SECTION\n")
    coordinates = []
    demands = []
    for i in range(coord_section_line, demand_section_line - 1):
        line = lines[i].strip().split()
        coordinates.append([int(line[1]), int(line[2])])
    for i in range(demand_section_line, depot_section_line):
        line = lines[i].strip().split()
        demands.append(int(line[1]))
    vehicle_cap = int(lines[coord_section_line-2].strip().split()[-1])
    return coordinates, demands, vehicle_cap
