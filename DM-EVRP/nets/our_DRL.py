...
class AttentionModel(nn.Module):
    ...
    def _init_embed(self, input):
        #  input[batch, node_len, 3] 第三个维度的信息包括 x，y，demand
        return torch.cat(
            (
    ...
    def route_feature(self, tour, dynamic, embeddings):
        # 处理路径及车辆特征
        ...
        if tour:
            ...
            tour_con = torch.gather(
                ...
                1,
                (batch_tour.clone())[..., None].contiguous()  # [batch_size, tour_len]
                    .expand(batch_size, batch_tour.size(-1), embed_dim)
            ).view(batch_size, batch_tour.size(-1), embed_dim)  # [batch_size, tour_len, embed_dim]
            mean_tour = torch.max(tour_con, dim=1)[0]
            veh_context = torch.cat(                              # [batch, 3]
                (
                    dynamic[:, 0:1, 0],                           # [batch_size, 1] load
                    dynamic[:, 2:3, 0] / self.start_soc,          # [batch_size, 1] soc
                    dynamic[:, 3:4, 0] / self.t_limit             # [batch_size, 1]time
                ),
                -1
            )
        else:
            ...
            veh_context = torch.cat(
                (
                    dynamic[:, 0:1, 0],
                    dynamic[:, 2:3, 0] / self.start_soc,
                    dynamic[:, 3:4, 0] / self.t_limit
                ),
                -1
            )
        veh_context1 = self.tour(veh_context)
        cat_context = torch.cat((veh_context1, mean_tour), -1).view(batch_size, self.embedding_dim * 2)
        context = self.FF_tour(cat_context)
    ...
    def _inner(self, information, distances, slopes, embeddings):
        ...
        return torch.stack(outputs, 1), torch.stack(tour_idx, 1), R, E
    ...
    def sample_many(self, input, batch_rep=1, iter_rep=1):
        ...
        for i in range(iter_rep):
            tour_idx, tour_logp, R, E = self.forward(batch)  # tour_idx的形状为[batch,sequence_len]
    ...
    def _get_log_p(self, fixed,  now_idx, mask, route_feature, embeddings):
        ...
        query = fixed.context_node_projected + route_feature + now_embedding
