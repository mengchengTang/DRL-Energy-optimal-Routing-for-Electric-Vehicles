...
class VehicleRouting():
    ...
    def build_model(self,):
        ...
        #                                  for i, j in A if i != j), GRB.MINIMIZE)  # 最小化能耗
        self.model.setObjective(quicksum( c[i,j] * x[i, j] for i, j in A if i != j), GRB.MINIMIZE) # 最小化距离
        # 添加约束
        # 约束一：每个客户点只能访问一次
...
if __name__ == '__main__':
    ...
    # 图上参数
    parser.add_argument('--nodes',  default=22, type=int)
    ...
    parser.add_argument('--plot_num', default=0,  help='whether plot')
    args = parser.parse_args()

    # filename = os.path.join("..", "test_data",f"{args.nodes}", "128_seed12345.pkl")  # 上级目录用..
    filename = os.path.join("..", "test_data", "CVRPlib", "P-n23-k8.txt.pkl")
    ...
    if not args.CVRP_lib_test:
        ...
        now = now.replace(':', '_')
        output_path = os.path.join("data_record", "10", "gurobi", f"{now}.csv")
