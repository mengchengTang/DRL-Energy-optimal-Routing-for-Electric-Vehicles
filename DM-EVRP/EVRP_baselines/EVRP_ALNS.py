...
class EVRP():
    def __init__(self, i, instance, args):
        ...
        num_dict = {15:10, 25:20, 59:50, 109:50} # 每一个问题规模对应的客户人数
    ...
    def split_routes(self, node_seq):
        ...
        for i in range(1, len(node_seq)):
            ...
            if check_load:
                # 充电去往下一个点，在回到仓库
                check_nextnode_soc = (soc >= (
                            self.Soc_Consume(node_seq[i - 1], node_seq[i], load) + self.Soc_Consume(node_seq[i], 0,
                                                                                                    load - self.demands[
                                                                                                        node_seq[i]])))
                if check_nextnode_soc:
                    # 满足能耗的时候，再确定时间约束能不能满足
                    check_nextnode_time = ((time + (
                                self.distances[node_seq[i - 1], node_seq[i]] / self.velocity) + self.custom_time + (
                                                        self.distances[node_seq[i], 0] / self.velocity)) <= self.t_limit)
                    if check_nextnode_time:
                        ...
                        distance_list.append(self.distances[node_seq[i - 1], node_seq[i]])
                        ...
                        distance_list.append(self.distances[node_seq[i - 1], 0])
                        vehicle_soc_list.extend(soc_list)
                        route = [node_seq[i]]
                        num_vehicle = num_vehicle + 1
                        soc_list = [[self.Soc_Consume(0, node_seq[i], self.max_load), 0]]
                        distance_list.append(self.distances[0, node_seq[i]])
                    ...
                    if soc >= min_soc:  # 如果可以插入充电站 去完充电站当然可以回去
                        check_nextstation_time = ((time + (
                                    self.distances[node_seq[i - 1], min_index] / self.velocity) + self.charging_time +
                                                   (self.distances[min_index, 0] / self.velocity)) <= self.t_limit)
                        if check_nextstation_time:
                            # 满足了货物量、时间、能耗这几个要素才能被添加
                            route.append(min_index)
                            # 去更新去往充电站的状态更新
                            soc_list.append([self.Soc_Consume(node_seq[i - 1], min_index, load),
                                             soc - min_soc - self.Start_SOC])  # 这个要看去了充电站实际到底是多电了还是少电了,添加充电量的负数
                            distance_list.append(self.distances[node_seq[i - 1], min_index])
                            soc = self.Start_SOC
                            ...
                            check_nextnode_time = ((time + (
                                        self.distances[min_index, node_seq[i]] / self.velocity) + self.custom_time + (
                                                                self.distances[
                                                                    node_seq[i], 0] / self.velocity)) <= self.t_limit)
                            if check_nextnode_time:
                                route.append(node_seq[i])
                                # 更新去点的动态信息
                                soc = soc - self.Soc_Consume(min_index, node_seq[i], load)
                                soc_list.append([self.Soc_Consume(min_index, node_seq[i], load), 0])
                                distance_list.append(self.distances[min_index, node_seq[i]])
                                ...
                                distance_list.append(self.distances[min_index, 0])
                                vehicle_soc_list.extend(soc_list)
                                route = [node_seq[i]]
                                num_vehicle = num_vehicle + 1
                                soc = self.Start_SOC - self.Soc_Consume(0, node_seq[i], self.max_load)
                        ...
                        for j in range(1, len(route)):
                            ...
                            SOC_station1 = [self.Soc_Consume(node_seq[i - 1 - j], n, forward_load) for n in
                                            range(1, self.charge_num + 1)]
                            ...
                            if forward_soc >= min_soc1:
                                ...
                                check_nextstation_time = ((time - (self.distances[node_seq[i - 1 - j], node_seq[
                                    i - 1 - j + 1]] / self.velocity) + (self.distances[node_seq[i - 1 - j], min_index1] +
                                                                        self.distances[min_index1, node_seq[
                                                                            i - 1 - j + 1]]) / self.velocity + self.charging_time +
                                                           (self.distances[
                                                                node_seq[i - 1], 0] / self.velocity)) <= self.t_limit)
                                if check_nextstation_time:  # todo:还是要用insert，直接索引的话会替换
                                    route.insert(-j, min_index1)
                                    # route[-j-1] = min_index1
                                    # 去更新去往充电站的状态更新
                                    soc_list.insert(-j, [self.Soc_Consume(node_seq[i - 1 - j], min_index1, forward_load),
                                                         forward_soc - min_soc1 - self.Start_SOC])
                                    distance_list.insert(-j, self.distances[node_seq[i - 1 - j], min_index1])
                                    soc_list[-j] = [self.Soc_Consume(min_index1, node_seq[i - 1 - j + 1], forward_load), 0]
                                    distance_list[-j] = self.distances[min_index1, node_seq[i - 1 - j + 1]]
                                    time_list.insert(-j, self.distances[
                                        node_seq[i - 1 - j], min_index1] / self.velocity + self.charging_time)
                                    time_list[-j] = self.distances[min_index1, node_seq[
                                        i - 1 - j + 1]] / self.velocity + self.custom_time
                                    # 对当前node_seq[i-1]时刻的soc和时间进行更新
                                    soc = self.Start_SOC - np.array(soc_list).sum()
                                    time = np.array(time_list).sum()
                                    # 因为刚去了充电站，所以这里检查去客户点的时间
                                    check_nextnode_time = ((time + (
                                                self.distances[node_seq[i - 1], node_seq[i]] / self.velocity) + (
                                                                        self.distances[node_seq[
                                                                                           i], 0] / self.velocity) + self.custom_time) <= self.t_limit)
                                    if check_nextnode_time:
                                        route.append(node_seq[i])
                                        # 更新去点的动态信息
                                        soc = soc - self.Soc_Consume(node_seq[i - 1], node_seq[i], load)
                                        soc_list.append([self.Soc_Consume(node_seq[i - 1], node_seq[i], load), 0])
                                        distance_list.append(self.distances[node_seq[i - 1], node_seq[i]])
                                        time = time + (self.distances[
                                                           node_seq[i - 1], node_seq[i]] / self.velocity) + self.custom_time
                                        time_list.append((self.distances[node_seq[i - 1], node_seq[
                                            i]] / self.velocity) + self.custom_time)
                                        load = load - self.demands[node_seq[i]]
                                        break
                                    else:
                                        vehicle_routes.append(route)
        ...
        distance_list.append(self.distances[node_seq[i], 0])
        vehicle_soc_list.extend(soc_list)  # todo:还要加上从仓库出发再从最后一个点到达仓库的
        vehicle_soc = np.array(vehicle_soc_list)[:, 0].sum()
        vehicle_distance = np.array(distance_list).sum()
        return vehicle_distance, vehicle_routes, vehicle_soc_list
...
if __name__ == '__main__':
    ...
    # 图上参数
    parser.add_argument('--nodes',  default=50, type=int)
    ...
    parser.add_argument('--epochs', default=80, type=int, help='epoch of iteration')
    ...
    filename = os.path.join("..","test_data",f"{args.nodes}","128_seed12345.pkl")
    date = HCVRPDataset(filename, num_samples=128, offset=0)
