...
class EVRP():
    ...
    def Soc_Consume(self, i, j, load):
        ...
        power = (0.5 * self.Cd * self.A * self.Ad * (self.velocity / 3.6) ** 2 + (load * self.w + self.mc) * self.g * (
                    self.slope[i, j] + self.Cr))
                ...
                if check_nextnode_soc:
                    ...
                    if soc >= min_soc:
                        ...
                        if check_nextstation_time:
                            ...
                            soc_list.append(
                                [self.Soc_Consume(node_seq[i - 1], min_index, load), soc - min_soc - self.Start_SOC])
                        ...
                        for j in range(1, len(route)):
                            ...
                            if forward_soc >= min_soc1:
                                ...
                                check_nextstation_time = ((time - (self.distances[node_seq[i - 1 - j], node_seq[
                                    i - 1 - j + 1]] / self.velocity) + (
                                                                   self.distances[node_seq[i - 1 - j], min_index1] +
                                ...
                                if check_nextstation_time:
                                    ...
                                    soc_list.insert(-j,
                                                    [self.Soc_Consume(node_seq[i - 1 - j], min_index1, forward_load),
                                                     forward_soc - min_soc1 - self.Start_SOC])
                                    distance_list.insert(-j, self.distances[node_seq[i - 1 - j], min_index1])
                                    soc_list[-j] = [self.Soc_Consume(min_index1, node_seq[i - 1 - j + 1], forward_load),
                                                    0]
                                    ...
                                    if check_nextnode_time:
                                        ...
                                        soc_list.append([self.Soc_Consume(node_seq[i - 1], node_seq[i], load), 0])
                                        distance_list.append(self.distances[node_seq[i - 1], node_seq[i]])
                                        time = time + (self.distances[node_seq[i - 1], node_seq[
                                        ...
                                        time = (self.distances[0, node_seq[i]] / self.velocity) + self.custom_time
                                        time_list = [
                                            (self.distances[0, node_seq[i]] / self.velocity) + self.custom_time]
                                        load = self.max_load - self.demands[node_seq[i]]
                                        break
                                else:
                                    vehicle_routes.append(route)
    ...
    def createRandomDestory(self, ):
        ...
        reomve_list = random.sample(range(self.charge_num + 1, self.custom_num + self.charge_num + 1),
                                    int(d * self.custom_num))
        return reomve_list
